...
class Graph(object):
	...
	edges needs to be removed from the complete graph.
	"""
	...
	def add_edge(self, node1, node2, cost = None, heuristic = None, direction = True):
		...
		direction equal to True implies that edge is directed, otherwise undirected. 
		Precondition: direction is boolean (Default value is True).
		"""
	...
	def _is_connected(self, visited_node_lst):
		"""
	...
	def randomGraphCreater(self, dropout):
		...
		while len(self._edges) > (length - num_edges_to_drop):
			...
			if G_temp._is_connected([]):                             # checking whether graph is connected or not after deleting edge
				self = G_temp
